...
# Defers to the workspace launcher so that runtime env loading can be edited
# without rebuilding the image. WORKSPACE_ROOT is provided by
...
if [[ ! -x "$WRAPPER" ]]; then
    ...
    echo "  Refusing to run without the workspace launcher." >&2
    exit 1
fi
